...
Client : Ce qui se conect au server et qui n'est pas un server. Son nickname fait 9 caracteres maximum. Le server doit connaitre : le vrai nom de l'host sur lequel le client est, l'username du client et le server sur  lequel le client est connecté.

Operator: Il peut connecté et deconnecté le server.  Il peut :

| Commande | Parametre                                                            | Fonction                                                                                          | Autre                                                         |
| -------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| KICK     | `<channel> <user> [<comment>]`                                       | Supprimer un user d'un ch                                                                         |                                                               |
...
i: Permet de mettre le channel em mode inviter et donc de pouvoir utiliser la commande INVITE
